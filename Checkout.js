...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('nombreMedico').addEventListener('focus', function() {
    ...
    document.getElementById('nombreMedico').addEventListener('blur', function() {
        const disclaimer = this.parentNode.querySelector('.disclaimer-medico');
        if (disclaimer) {
...
function cargarResumenPedido() {
    ...
    // 1. Intentar desde parámetros URL primero (para GitHub Pages)
    try {
    ...
    if (!carritoData) {
        try {
            ...
            if (localData) {
                ...
                // Verificar que los datos vengan del mismo origen
...
function procesarCheckout(e) {
    ...
    setTimeout(() => {
        ...
        const orderData = {
            ...
            cart: JSON.parse(localStorage.getItem('carritoCheckout')).items,
