...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cargar el resumen del pedido después de un pequeño delay
    setTimeout(() => {
        cargarResumenPedido();
        if (loadingElement) {
...
function cargarResumenPedido() {
    ...
    try {
        ...
        if (localData) {
            carritoData = JSON.parse(localData);
        }
    } catch (e) {
    ...
    if (!carritoData) {
        try {
            ...
            if (sessionData) {
                carritoData = JSON.parse(sessionData);
            }
        } catch (e) {
    ...
    // Si aún no hay datos, intentar con cookies
    if (!carritoData) {
        try {
            ...
            if (cookieValue) {
                carritoData = JSON.parse(decodeURIComponent(cookieValue));
            }
        } catch (e) {
